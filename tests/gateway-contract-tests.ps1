...
Assert-Contains $variables 'variable "authorizer_id"' 'The platform must accept the reviewed FUN authorizer ID as an optional handoff input.'
Assert-Contains $module 'if contains(local.public_app_routes, route_key) || var.authorizer_id != null' 'Protected routes must remain absent until the FUN authorizer handoff is supplied.'
