...
try{
 ...
 if($absent -eq 8){
   ...
   foreach($object in $bundle.objects){
     ...
     $null=Kube @('create','-f',$path)
     $actual=ReadObject $object;if($null -eq $actual){throw 'PREREQUISITES_CREATE_READBACK_MISSING'};$readback+=$actual
 ...
 $receipt=[pscustomobject]@{schemaVersion=1;environment='staging';appSourceCommit=$AppSourceCommit;k8sSourceCommit=$K8sSourceCommit;bundleSha256=$ExpectedBundleSha256;sourceSha256=$ExpectedSourceSha256;windowSha256=$ExpectedWindowSha256;status=$status;objects=$readback;completedAtUtc=[DateTimeOffset]::UtcNow.ToString('o')}
