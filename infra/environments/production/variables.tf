...
variable "authorizer_handoff" {
  ...
  validation {
    condition = var.authorizer_handoff == null || (
      ...
      var.authorizer_handoff.environment == "production"
    )
    error_message = "authorizer_handoff must contain the reviewed same-account API ID, execution ARN, authorizer ID and production environment."
  }
}
