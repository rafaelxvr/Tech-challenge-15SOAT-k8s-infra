...
variable "network" { type = object({ private_subnet_ids = set(string), function_security_group_id = string }) }
...
variable "runtime_secret_arns" {
  type = object({ auth_lookup = string, notification_lookup = string, customer_signing_key = string, customer_public_keys = string, staff_hmac = string })
}
variable "customer_key_id" { type = string }
variable "staff_key_id" { type = string }
variable "database" { type = object({ host = string, port = number, name = string, ca_path = string }) }
