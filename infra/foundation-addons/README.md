...
VPC provisioning also needs `ec2:CreateNetworkInterfacePermission`, separately from the existing seven EC2 lifecycle/describe actions. Both the addons and normal deployment executors grant this action only on their configured region/account `network-interface/*` ARNs, with `ec2:AuthorizedService = codebuild.amazonaws.com` and `ec2:Subnet` restricted to the configured private-subnet ARNs. The action is never added to the `Resource = "*"` base list. This follows [AWS's CodeBuild VPC service-role example](https://docs.aws.amazon.com/codebuild/latest/userguide/auth-and-access-control-iam-identity-based-access-control.html). Its absence can prevent provisioning with `VPC_CLIENT_ERROR` / `UnauthorizedOperation`; the offline policy contracts verify the required scope, while a reviewed IAM rollout and staging retry are still needed to confirm the live result.
