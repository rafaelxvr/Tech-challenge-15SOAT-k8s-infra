...
`images/deployer/Dockerfile` is the platform image definition. It pins the Amazon Linux base digest and defaults to PowerShell 7.4.6, Java 17, AWS CLI 2.36.42, Terraform 1.15.8, kubectl 1.35.0 and Helm 3.17.4. Its five tool downloads require reviewed SHA256 inputs and are checked before use; DNF package resolution remains mutable. The image build/publish workflow is not yet implemented: see the [dependency verification and future workflow contract](deployer-image-verification.md) for builder identity, SBOM/provenance, scan disposition and unresolved package inputs. CodeBuild consumes only a reviewed ECR digest, not a mutable tag. The ECR policy retains one image; that image counts against the previously approved retained-image allowance.
