...
The K8S platform state is the single owner for the base API, VPC-link integrations, APP target group/routes and stage. Its first apply omits protected APP routes when `authorizer_handoff` is null. FUN receives the K8S API ID/execution ARN and owns only the REQUEST authorizer, the two public CPF routes and their exact invoke permissions. The reviewed FUN output receipt must conform to [`contracts/gateway-handoff.schema.json`](../contracts/gateway-handoff.schema.json), and the second K8S apply consumes that complete object. K8S validates the account/region/environment shape and checks the attested API ID/execution ARN against its API before creating protected routes. No state may create a duplicate API, route or Lambda permission.
