...
| OIDC trust | Reviewed IAM trust policy with `aud=sts.amazonaws.com` and the exact `repo:OWNER@OWNER_ID/REPOSITORY@REPOSITORY_ID:environment:ENVIRONMENT` subject from reviewed GitHub OIDC configuration | `pull_request`, wildcard repository, or wildcard environment subjects are rejected. |
...
After a reviewed foundation apply, its distinct foundation execution identity receives only the exported `foundation_output_publisher_policy_arn`. It runs `scripts/publish-foundation-outputs.ps1`, which exports the allowlist, writes it to `releases/k8s/foundation/outputs/{sourceCommit}.json`, and records the returned S3 `VersionId` and SHA-256 in a foundation-output receipt. Kubernetes and reviewed database staging/production launchers receive only GetObject/GetObjectVersion access to that prefix; APP and functions receive no new grant without a proven receipt dependency. Database access also requires the exact reviewed repository and releases/database/{environment} source prefix. Their protected GitHub environment stores the reviewed receipt JSON as `FOUNDATION_OUTPUT_RECEIPT_JSON`; each repository's `scripts/resolve-foundation-outputs.ps1` retrieves the exact named version and verifies the receipt digest/schema/environment/source commit. The Kubernetes resolver adds `foundation_outputs` to platform tfvars and rejects base tfvars that already supply that field. The database resolver instead adds `vpc_id` and `database_subnet_ids` to database tfvars.
