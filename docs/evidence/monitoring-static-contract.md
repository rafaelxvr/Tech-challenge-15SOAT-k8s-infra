...
Authorized activation still needs reviewed environment/endpoint/provider inputs, valid cloud access, populated telemetry, observed latency/resource thresholds and meaningful status transitions. Validate the live NRQL results, JSON field ingestion, request-to-trace navigation, both health responses and a bounded synthetic failure. The optional [staging delivery contract](monitoring-alert-delivery-contract.md) now defines a destination/channel/workflow with an explicit protected recipient and a disabled default; actual alert delivery remains unverified. Keep live dashboard/alert delivery, runtime readiness and R4 acceptance pending until that evidence exists.
