...
./scripts/render-platform.ps1 -Environment staging `
  ...
  -DbHost 'DATABASE_ENDPOINT' -DbCidr @('10.42.64.0/24', '10.42.65.0/24') `
...
Every namespace begins with ingress/egress deny. App ingress is limited to the two ALB source subnet CIDRs, same-environment app pods and metrics-server. It cannot admit arbitrary VPC pod traffic, so staging app pods cannot reach production app pods on port 8080. CoreDNS and metrics-server peers combine the exact `kube-system` namespace selector with their `k8s-app` pod label; they must not require oficina ownership labels. APP workload, Service and policy selectors retain their explicit ownership labels.

Egress permits CoreDNS, the exact union of supplied database CIDRs on TCP 5432 and HTTPS for approved AWS services. `-DbCidr` accepts an array (also named `-DatabaseCidrs`) and retains single-CIDR compatibility. The staging example uses both reviewed RDS subnets, `10.42.64.0/24` and `10.42.65.0/24`; these are explicit inputs, never defaults or a widened supernet. Production callers must supply their separately reviewed subnet inputs. Duplicate, malformed and empty inputs are rejected. Standard Kubernetes `NetworkPolicy` cannot identify AWS services by FQDN, so HTTPS egress remains additionally bounded by environment IRSA, private-subnet routing and AWS security groups. `namespace-isolation.ps1 -Run` uses the deployed `oficina-staging`/`oficina-production` policies and an app-labeled probe, and refuses a non-Cilium cluster. These are static rendering checks; R4 must verify the result using live endpoints, and this document does not claim FQDN-level filtering or a successful rollout.
